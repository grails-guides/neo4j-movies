...
Now let's implement the controller action that will invoke this method. First autowire the service into the `MovieController` class:
...
This using Spring to inject the dependency and make the service implementation available. Next add an action called `show` that invokes the `find` method and responds with the result:
