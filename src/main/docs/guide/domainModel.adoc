...
The `CastMember` domain class is going to model the relationship between a `Person` and a `Movie`. To do that we are going to use the `grails.neo4j.Relationship` trait:
